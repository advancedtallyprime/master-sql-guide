# Chapter 17 — Schema Abstraction & Security: MySQL Views (MySQL Views)

---

## 1. What is it? (Ye Kya Hai?)

MySQL me **View** ek named, saved `SELECT` query hoti hai jo database dictionary ke andar store rehti hai aur ek **Virtual Table** ki tarah kaam karti hai.

Normal relational base tables ki tarah, view disk par actual physical data rows store **nahi** karta aur na hi koi alag disk space leta hai (sirf data dictionary me ek chhoti si definition file rehti hai). Jab bhi koi application ya user kisi view ko query karta hai, MySQL ka query optimizer view ki underlying query definition ko user ki outer query ke sath dynamically merge kar deta hai. Isse ek combined execution plan banta hai jo seedhe underlying base tables se live data fetch karta hai!

Kyunki view access hone ke theek usi second par execute hota hai, isliye **view hamesha 100% up-to-date rehta hai**: base tables me koi bhi data insert, update ya delete hote hi, agle hi pal view query me wo live change reflect ho jata hai.
...
2. **Complex Queries Ki Simplification**: Developers ya reporting tools ko baar-baar 6-table joins aur complex `CASE` statements likhne ki zaroorat nahi hai. Poora complex logic ek view (jaise `sales_summary_view`) me daal dijiye, aur clients simple query run kar sakte hain: `SELECT * FROM sales_summary_view WHERE region = 'EMEA'`.
...
4. **Consistent Business Metrics**: Company me "Active High-Value Customer" ya "Monthly Net Revenue" ka formula Finance, Marketing aur Operations team ke beech hamesha same rehna chahiye. Jab ye business logic ek shared view me define hota hai, toh sabhi departments hamesha identical numbers report karte hain.
...
MySQL kuch simple views me seedhe `INSERT`, `UPDATE`, aur `DELETE` execute karne ki permission deta hai, jo changes seedhe base table par pass ho jaate hain.
...
Hamare `sql_mastery` enterprise database me, finance aur inventory teams ke liye do critical views design karte hain:
...
1. `CREATE OR REPLACE VIEW v_order_financial_summary AS ...`:
   ...
   * View ka definition data dictionary metadata me save ho jata hai; disk par koi data pages materialize nahi hote.
2. `SELECT ... FROM v_order_financial_summary WHERE outstanding_balance > 0;`:
   * Query optimizer view definition ko fetch karta hai aur bahar lage filter `outstanding_balance > 0` ko seedhe underlying query pipeline me merge kar deta hai.
...
1. **Yeh Sochna Ki View Data Store Karta Hai (Performance Misconception)**:
   ...
   * *Reality*: Standard MySQL me views strictly virtual hote hain. View ko query karne ka matlab hai uski underlying query ko har baar naye sire se execute karna. Agar underlying query slow hai, toh view bhi utna hi slow chalega!
2. **Non-Updatable Views Ko Update Karne Ki Koshish Karna**:
   * Aise views par `UPDATE` ya `DELETE` chalana jisme neeche diye gaye elements shamil hon:
...
3. **Base Table Drop Kar Dena Jabki View Us Par Dependent Ho**:
   * Agar aap `DROP TABLE customers;` chala dete hain, toh view catalog me bacha reh jata hai. Lekin jaise hi koi user us view ko query karega, fatal error aayega:
...
1. **View Definition Me Explicit Column Names Use Karein**:
   * View create karte waqt kabhi bhi `SELECT *` na likhein! Agar bhavishya me base table me koi column add ya drop hota hai, toh `SELECT *` wala view desynchronized ya corrupt ho sakta hai. Hamesha har column ka naam explicitly mention karein (`SELECT id, name, email...`).
...
### Easy
1. Ek aisa view `v_us_customers` banaiye jo sirf `'USA'` country ke customers ke `customer_id`, `first_name`, `last_name`, aur `city` columns ko show kare.
...
5. Prove kijiye ki view 100% dynamic hota hai: pehle `v_product_stock_status` ko query karein, fir underlying `products` table me stock quantity update karein, aur dobara view ko query karke live update verify karein.
...
### Difficult
7. MySQL me view creation ke waqt `ALGORITHM = MERGE` aur `ALGORITHM = TEMPTABLE` ke beech kya antar hota hai? Kaun sa algorithm view ko updatable banata hai aur kyun?
8. Kyunki MySQL me Oracle ya PostgreSQL ki tarah native Materialized Views ka support nahi hota, aap MySQL me heavy sales summary ke liye Materialized View pattern kaise emulate karenge (Physical Table + Stored Procedure + MySQL Event Scheduler ka use karke)?

---

## 11. Interview Questions (Interview Questions)

### Q1: SQL me View kya hota hai aur kya ye disk storage consume karta hai?
**Answer**: View ek saved SQL query dwara define ki gayi virtual table hoti hai. Ye disk par actual data rows ya data pages store nahi karta, isliye ye koi data storage consume nahi karta. Jab kisi view ko query kiya jata hai, database engine view ki underlying query ko dynamically run karta hai (ya `ALGORITHM = MERGE` ke sath outer query ke sath merge kar deta hai) aur underlying physical tables se live data fetch karta hai. View sirf data dictionary me apna schema definition text store karne ke liye negligible space leta hai.

### Q2: MySQL me kab koi View Updatable hota hai aur kab Non-Updatable?
...
Neeche diye gaye cases me view **kabhi bhi updatable nahi hota**:
...
7. Specific contexts me `LIMIT` clause ka use
...
Agar `WITH CHECK OPTION` na lagaya jaye, toh agar koi view `WHERE country = 'USA'` par bana hai, toh koi user us view ke zariye country ko `'Germany'` update kar sakta hai. Update base table me ho jayega lekin row view se gayab ho jayegi ("ghost update"). `WITH CHECK OPTION` lagane par MySQL modified row ko evaluate karta hai aur agar wo criteria satisfy nahi karti, toh statement ko foran abort karke error throw kar deta hai.
...
* Agar view me `GROUP BY`, `DISTINCT`, `UNION`, ya aggregate functions hon, toh wo **non-updatable** ban jata hai.
